...
# Training arguments
training_args=(
  --json_path $DATA_DIR/train_wdsa.json
  ...
  --window_frames 24
  --tracker_project_name temporal_embed
  --output_dir outputs/butter4_temporal_embed_v2
  --max_train_steps 500
  --train_batch_size 1
  ...
  --logit_mean 0.0
  --logit_std 1.0
  # Temporal embedding training flags
  --temporal_embed_training
  --temporal_embed_max_frames 30
  # Video generation eval every 50 steps
  --log_validation
  --validation_steps 30
  --eval_pose_json $DATA_DIR/w_d_s_a/pose.json
  --eval_image_path /data/ziqi/Repos/HY-WorldPlay-New/training_data/butter4_gt/w_d_s_a.mp4
...
# Optimizer arguments — higher LR since only 200K params are trained
optimizer_args=(
  --learning_rate 1e-3
