...
class FlowMatchDiscreteScheduler(SchedulerMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.timesteps = (sigmas[:-1] * num_train_timesteps).to(
            dtype=torch.float32
        )
    ...
    def set_begin_index(self, begin_index: int = 0):
        """Sets the begin index for the scheduler. This function should be run from pipeline before
        the inference.
    ...
    def step(
        ...
        propagates the diffusion process from the learned model outputs (most often the predicted
        noise).
