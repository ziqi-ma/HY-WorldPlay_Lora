...
from fastvideo.models.hyvideo.commons import PRECISION_TO_TYPE
...
def load_vision_encoder(
    ...
    if vision_encoder_type == "siglip":
        ...
        raise ValueError(
            f"Unsupported vision encoder type: {vision_encoder_type}"
        )
...
class VisionEncoder(nn.Module):
    def __init__(
        ...
        self.processor_type = (
            ...
            else vision_encoder_type
        )
        self.processor_path = (
            processor_path
            if processor_path is not None
            else vision_encoder_path
        )
        self.logger = logger
    ...
    def __repr__(self):
        return (
            f"{self.vision_encoder_type} ({self.precision} - {self.model_path})"
        )
    ...
    def encode_latents_to_images(self, latents, vae, reorg_token=False):
        ...
        first_image_latents = (
            1 / vae.config.scaling_factor * first_image_latents
        )
    ...
    def encode_latents(self, latents, vae, reorg_token=False):
        """Encode latents by first converting to images, then encoding. This is the main function
        that replaces sigclip_vision_encode.
