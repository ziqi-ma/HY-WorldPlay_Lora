...
def importance_sampling(
    q, k, topk, threshold=0.0, lambda_=0.9, adaptive_pool=None
):
    ...
    if threshold > 0.0:
        raise NotImplementedError(
            "importance_sampling with threshold not implemented"
        )
    ...
    importance_scores = (
        lambda_ * gate_similarity - (1 - lambda_) * mean_redundancy
    )
...
def similarity_sampling(
    ...
    if threshold > 0.0:
        ...
        indices = torch.arange(gate.size(-1), device=gate.device).expand(
            gate.size()
        )
...
def create_moba_3d_mask(
        ...
        if adaptive_pool is not None:
            ...
            if pooling_type == "avg":
                ...
                global_avg = global_avg / (
                    global_avg.norm(dim=1, keepdim=True) + 1e-8
                )
                global_avg = global_avg.permute(0, 2, 3, 4, 1)
                global_avg = global_avg.reshape(
                ...
                max_pool = max_pool / (
                    max_pool.norm(dim=1, keepdim=True) + 1e-8
                )
                max_pool = max_pool.permute(0, 2, 3, 4, 1)
                max_pool = max_pool.reshape(
                    batch_size,
                ...
                raise ValueError(
                    f"pooling_type={pooling_type} is not Supported"
                )
        else:
    ...
    if (
        sampling_type == "similarity" and threshold > 0.0
    ) and adaptive_pool is None:
        adaptive_pool = (2, 2, 2)
    ...
    if sampling_type == "similarity":
        ...
        raise NotImplementedError(
            f"sampling_type={sampling_type} is not Supported"
        )

    q = q.type_as(k)
    # Keep head dimension to dynamically build mask
    assert (
        top_block_indices.size(0) == 1
    ), "top_block_indices batch size must be 1"
    ...
    for head_idx in range(top_block_indices.size(0)):
        ...
        head_mask.scatter_(
            dim=-1, index=top_block_indices[head_idx], value=True
        )
...
def create_sta_3d_mask_optimize(canvas_thw, tile_thw, kernel_thw):
    ...
    kv_h_tile = (j_grid % (tile_thw_num[1] * tile_thw_num[2])) // tile_thw_num[
        2
    ]
...
def create_ssta_3d_mask(
    ...
    """
    sta_3d_mask = create_sta_3d_mask(
...
def ssta_3d_attention(
    ...
    if t % tile_t != 0 or h % tile_h != 0 or w % tile_w != 0:
        ...
        if pad_t > 0:
            t = t + pad_t
            repeat_q = pad_image_q[:, :, -1:, :, :, :].expand(
                -1, -1, pad_t, -1, -1, -1
            )
            repeat_k = pad_image_k[:, :, -1:, :, :, :].expand(
                -1, -1, pad_t, -1, -1, -1
            )
            repeat_v = pad_image_v[:, :, -1:, :, :, :].expand(
                -1, -1, pad_t, -1, -1, -1
            )
            if pad_type == "zero":
    ...
    if text_len % block_size > 0:
        ...
        pad_text_q = (
            text_q[:, :, -1, :].unsqueeze(2).expand(-1, -1, text_pad_size, -1)
        )
        pad_text_k = (
            text_k[:, :, -1, :].unsqueeze(2).expand(-1, -1, text_pad_size, -1)
        )
        pad_text_v = (
            text_v[:, :, -1, :].unsqueeze(2).expand(-1, -1, text_pad_size, -1)
        )
