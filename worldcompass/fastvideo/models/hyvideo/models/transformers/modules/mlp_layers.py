...
from fastvideo.models.hyvideo.commons import to_2tuple
...
class LinearWarpforSingle(nn.Module):
    def __init__(
        self, in_dim: int, out_dim: int, bias=False, device=None, dtype=None
    ):
        factory_kwargs = {"device": device, "dtype": dtype}
        super().__init__()
...
class FinalLayer(nn.Module):
    ...
    def __init__(
        ...
        self.adaLN_modulation = nn.Sequential(
            act_layer(),
            nn.Linear(
                hidden_size, 2 * hidden_size, bias=True, **factory_kwargs
            ),
        )
