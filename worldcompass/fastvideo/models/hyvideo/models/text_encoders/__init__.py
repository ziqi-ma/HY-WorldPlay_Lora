...
class TextEncoder(nn.Module):
    ...
    def __repr__(self):
        return (
            f"{self.text_encoder_type} ({self.precision} - {self.model_path})"
        )
    ...
    def calculate_crop_start(self, tokenized_input):
        ...
        marker_tokens = self.tokenizer(marker, add_special_tokens=False)[
            "input_ids"
        ]
        ...
        if hasattr(self.tokenizer, "special_tokens_map"):
            ...
            for (
                token_name,
                token_value,
            ) in self.tokenizer.special_tokens_map.items():
                if "user" in token_name.lower():
                    user_token_id = self.tokenizer.convert_tokens_to_ids(
                        token_value
                    )
