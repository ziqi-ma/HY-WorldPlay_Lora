...
This module provides reusable validation functions that can be used across all pipeline stages for
...
class StageValidators:
    ...
    def list_of_tensors_with_min_dims(value: Any, min_dims: int) -> bool:
        """Check if value is a non-empty list where all items are tensors with at least min_dims
        dimensions and no NaN values."""
        if not isinstance(value, list) or len(value) == 0:
            return False
        for item in value:
            if not isinstance(item, torch.Tensor):
    ...
    def none_or_tensor_with_dims(dims: int) -> Callable[[Any], bool]:
        """Return a validator that checks if value is None or a tensor with specific dimensions and
        no NaN values."""
    ...
    def min_dims(min_dims: int) -> Callable[[Any], bool]:
        """Return a validator that checks if tensor has at least min_dims dimensions and no NaN
        values."""
    ...
    def list_of_tensors_dims(dims: int) -> Callable[[Any], bool]:
        """Return a validator that checks if value is a list of tensors with specific dimensions and
        no NaN values."""
    ...
    def list_of_tensors_min_dims(min_dims: int) -> Callable[[Any], bool]:
        """Return a validator that checks if value is a list of tensors with at least min_dims
        dimensions and no NaN values."""
    ...
    def _create_validation_failure(
        ...
        if validator_name == "is_tensor":
            ...
            if (
                ...
                and torch.isnan(value).any().item()
            ):
            ...
            "tensor_with_dims" in validator_name
            or "tensor_min_dims" in validator_name
        ):
            if isinstance(value, torch.Tensor):
