    ...
    **kwargs : dict
    ...
    if mode == "STA_searching":
        ...
        if (
            ...
            raise ValueError(
                "mask_search_files_path_pos, mask_search_files_path_neg,"
                " and save_dir are required for STA_tuning_cfg mode"
            )
        ...
        averaged_results = average_head_losses(
            combined_results, selected_masks_cfg
        )
...
def select_best_mask_strategy(
    ...
    strategy_counts: dict[str, int] = {
        str(strategy): 0 for strategy in selected_masks
    }
            ...
            for h in range(head_num):
                if t < skip_time_steps:  # First steps use full attention
                    ...
                    for strategy in selected_masks[
                        :-1
                    ]:  # Exclude full attention
                        head_losses.append(
                            averaged_results[loss_type][str(strategy)][t][
                                layer_idx
                            ][h]
                        )
