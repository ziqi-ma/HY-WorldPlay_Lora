...
def pad_vocab_size(
    vocab_size: int, pad_to: int = DEFAULT_VOCAB_PADDING_SIZE
) -> int:
...
class VocabParallelEmbeddingShardIndices:
    ...
    def num_org_elements_padded(self) -> int:
        return (
            self.padded_org_vocab_end_index - self.padded_org_vocab_start_index
        )
    ...
    def __post_init__(self):
        # sanity checks
        assert (
            self.padded_org_vocab_start_index <= self.padded_org_vocab_end_index
        )
        assert (
            self.padded_added_vocab_start_index
            <= self.padded_added_vocab_end_index
        ...
        assert (
            self.added_vocab_start_index <= self.padded_added_vocab_start_index
        )
...
class VocabParallelEmbedding(torch.nn.Module):
    ...
    def _get_indices(
        ...
        """Get start and end indices for vocab parallel embedding, following the layout outlined in
        the class docstring, based on the given tp_rank and tp_size."""
        ...
        # remove padding
        org_vocab_start_index = min(
            padded_org_vocab_start_index, org_vocab_size
        )
        org_vocab_end_index = min(padded_org_vocab_end_index, org_vocab_size)
        added_vocab_start_index = min(
    ...
    def get_sharded_to_full_mapping(self) -> list[int] | None:
        ...
        During sampling, we gather logits from all ranks. The relationship of index->token_id will
        follow the same format as outlined in the class docstring. However, after the gather, we
        want to reindex the final logits tensor to map index->token_id one-to-one (the index is
        always equal the token_id it corresponds to). The indices returned by this method allow us
        to do that.
        """
        if self.tp_size < 2:
            return None
