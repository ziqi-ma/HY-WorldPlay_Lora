...
def calculate_in_frustum_mask(
    ...
    """A function that takes in the depth, intrinsics and c2w matrices of two sets of views, and
    then works out which of the pixels in the first set of views has a direct corresponding pixel in
    any of views in the second set.
...
def unproject_depth(depth, intrinsics, c2w):
    ...
    world_points = camera_space_to_world_space(
        camera_points, c2w
    )  # (..., h, w, 3)
